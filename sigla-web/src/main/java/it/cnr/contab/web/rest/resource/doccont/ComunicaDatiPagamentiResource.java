...
public class ComunicaDatiPagamentiResource implements ComunicaDatiPagamentiLocal {
    ...
    public Response recuperoDatiPagamenti(@Context HttpServletRequest request, @QueryParam("esercizio") Integer esercizio,
        ...
        Optional.ofNullable(daData).orElseThrow(() -> new RestException(Response.Status.BAD_REQUEST, "Errore, data di inizio selezione obbligatoria."));
        Optional.ofNullable(aData).orElseThrow(() -> new RestException(Response.Status.BAD_REQUEST, "Errore, data di fine selezione obbligatoria."));
