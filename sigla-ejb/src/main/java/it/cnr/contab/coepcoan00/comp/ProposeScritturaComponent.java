...
public class ProposeScritturaComponent extends CRUDComponent {
	...
	private static class DettaglioFinanziario {
		...
		public DettaglioFinanziario(IDocumentoCogeBulk docamm, IDocumentoDetailEcoCogeBulk rigaDocamm, Integer cdTerzo, Voce_epBulk voceEp, BigDecimal imImponibile, BigDecimal imImposta, List<DettaglioAnalitico> dettagliAnalitici) {
			super();
			this.docamm = docamm;
			this.voceEp = voceEp;
			this.dettagliAnalitici = dettagliAnalitici;
		...
		public DettaglioFinanziario(IDocumentoCogeBulk docamm, IDocumentoDetailEcoCogeBulk rigaDocamm, Integer cdTerzo, Voce_epBulk voceEp, BigDecimal imImponibile, BigDecimal imImposta) {
			super();
			this.docamm = docamm;
            this.voceEp = voceEp;
			this.cdTerzo = cdTerzo;
	...
	private class TestataPrimaNota {
		...
		public DettaglioPrimaNota addDettaglio(UserContext userContext, String tipoDettaglio, String sezione, Voce_epBulk conto, BigDecimal importo, boolean isModificabile, boolean isAccorpabile) {
			return addDettaglio(userContext, tipoDettaglio, sezione, conto, importo, null, null, null, null, null, isModificabile, isAccorpabile);
		}
        ...
        try {
            ...
            if (doccoge.getTipoDocumentoEnum().isGenericoCoriVersamentoSpesa())
                ...
                if (((CompensoBulk) doccoge).getFl_compenso_stipendi())
                    throw new ScritturaPartitaDoppiaNotRequiredException("Scrittura Economica non prevista per la tipologia di documento selezionato.");
				if (((CompensoBulk) doccoge).isAnnullato())
					throw new ScritturaPartitaDoppiaNotRequiredException("Compenso " + ((CompensoBulk) doccoge).getEsercizio()+"/"+((CompensoBulk) doccoge).getCd_cds()+"/"+((CompensoBulk) doccoge).getPg_compenso() + " annullato. Registrazione economica non prevista.");
	...
	private List<TestataPrimaNota> proposeTestataPrimaNotaDocumento(UserContext userContext, IDocumentoAmministrativoBulk docamm) throws ComponentException, ScritturaPartitaDoppiaNotRequiredException, RemoteException, IntrospectionException, PersistencyException {
														...
														try {
															...
															if (isFatturaPassivaDaOrdini && !listaFatturaOrdini.isEmpty()) {
																for (DettaglioFinanziario rigaDettFin:righeDettFinVocePartita) {
																	...
																	Voce_ivaBulk pVoceIva = rigaDocamm.getVoce_iva();
																	if (pVoceIva!=null)
																		pVoceIva = (Voce_ivaBulk)fatpasHome.loadIfNeededObject(pVoceIva);
																	...
																	if (listaFatturaOrdiniCollRiga.isEmpty()) {
                                                                        ...
                                                                        for (FatturaOrdineBulk fatturaOrdineBulk:listaFatturaOrdiniCollRiga) {
																			...
																			//Se la riga ha tutti gli importi nulli (=0) salto la contabilizzazione della riga
																			if (fatturaOrdineBulk.getImponibilePerRigaFattura().compareTo(BigDecimal.ZERO)==0 &&
																				fatturaOrdineBulk.getImImponibile().compareTo(BigDecimal.ZERO)==0 &&
																				(!registraIvaACosto || (fatturaOrdineBulk.getImIva().compareTo(BigDecimal.ZERO)==0 &&
																						fatturaOrdineBulk.getIvaPerRigaFattura().compareTo(BigDecimal.ZERO)==0)))
																				continue;
																			...
																			//Se riga negativa allore cerco fatture da ricevere al contrario
																			if (fatturaOrdineBulk.getImImponibile().compareTo(BigDecimal.ZERO)<0)
																				movimentoFattRic = allMovimentiPrimaNota.stream().filter(Movimento_cogeBulk::isSezioneDare).findFirst().orElse(null);
																...
																righeDettFinVocePartita.forEach(rigaDettFin->{
																	try {
																		...
																		if (scritturaPartita.isPresent()) {
																			...
																			//in quel caso il costo è derivato tutto da quell'unica riga.....
																			Pair<Voce_epBulk, Voce_epBulk> pairContoCosto = this.findPairCostoRigaDocumento(userContext, rigaDettFin, pairContoCostoGen);
																			if (rigaDettFin.getRigaPartita().equals(rigaDettFin.getRigaDocamm())) {
		...
		if (isIntraUE || (isExtraUE && !isMerceIntraUE)) {
			...
			if (optLetteraPagamento.flatMap(el->Optional.ofNullable(el.getCd_sospeso())).isPresent() && !testataPrimaNotaList.isEmpty()) {
				...
				TestataPrimaNota testataPrimaNota = testataPrimaNotaList.getFirst();
				...
				final boolean ivaAdded = (((Fattura_passivaBulk)docamm).isIstituzionale() &&
						(((Fattura_passivaBulk)docamm).isFatturaDiServizi() ||
						(isIntraUE && ((Fattura_passivaBulk)docamm).isFatturaDiBeni())));

				Configurazione_cnrHome configHome = (Configurazione_cnrHome) getHome(userContext, Configurazione_cnrBulk.class);
				WorkpackageBulk gaeDefault = configHome.getGaeDocumentoNonLiquidabile(userContext, docamm);
				...
				for (Voce_epBulk conto : mapConto.keySet()) {
                    try {
						...
						BigDecimal totLetteraPagamento = optLetteraPagamento.get().getIm_pagamento();
						...
						if (ivaAdded) {
							...
							totLetteraPagamento = totLetteraPagamento.add(totIva);
						}

                        BigDecimal diffCambio = totLetteraPagamento.add(totStorni).subtract(totFattura);
                        if (diffCambio.compareTo(BigDecimal.ZERO) != 0) {
							...
							if (diffCambio.compareTo(BigDecimal.ZERO) > 0) {
								...
								DettaglioPrimaNota dettPN = testataPrimaNota.addDettaglio(userContext, Movimento_cogeBulk.TipoRiga.COSTO.value(), Movimento_cogeBulk.SEZIONE_DARE, contoCambio, diffCambio, DEFAULT_MODIFICABILE, Boolean.TRUE);
                                dettPN.setDettaglioFinanziario(new DettaglioFinanziario(docamm, contoCambio, diffCambio, BigDecimal.ZERO, new ArrayList<>()));
								List<Voce_analiticaBulk> voceAnaliticaList = ((Voce_analiticaHome) getHome(userContext, Voce_analiticaBulk.class)).findVoceAnaliticaList(contoCambio);
								...
								dettPN.getDettaglioFinanziario().getDettagliAnalitici().add(new DettaglioAnalitico(voceAnaliticaDef, gaeDefault.getCd_centro_responsabilita(), gaeDefault.getCd_linea_attivita(), diffCambio, dettPN));
								testataPrimaNota.openDettaglioPatrimonialePartita(userContext, docamm, docamm, pairContoCosto.getSecond(), diffCambio, terzoFattura.getCd_terzo(),
                                        DEFAULT_MODIFICABILE, Boolean.TRUE);
                            } else {
								...
								DettaglioPrimaNota dettPN = testataPrimaNota.addDettaglio(userContext, Movimento_cogeBulk.TipoRiga.RICAVO.value(), Movimento_cogeBulk.SEZIONE_AVERE, contoCambio, diffCambio.abs(), DEFAULT_MODIFICABILE, Boolean.TRUE);
								dettPN.setDettaglioFinanziario(new DettaglioFinanziario(docamm, contoCambio, diffCambio.abs(), BigDecimal.ZERO, new ArrayList<>()));
								List<Voce_analiticaBulk> voceAnaliticaList = ((Voce_analiticaHome) getHome(userContext, Voce_analiticaBulk.class)).findVoceAnaliticaList(contoCambio);
								if (voceAnaliticaList.isEmpty())
									return new ArrayList<>();
								Voce_analiticaBulk voceAnaliticaDef = voceAnaliticaList.stream()
										.filter(Voce_analiticaBulk::getFl_default).findAny()
										.orElse(voceAnaliticaList.stream().findAny().orElse(null));
								dettPN.getDettaglioFinanziario().getDettagliAnalitici().add(new DettaglioAnalitico(voceAnaliticaDef, gaeDefault.getCd_centro_responsabilita(), gaeDefault.getCd_linea_attivita(), diffCambio.abs(), dettPN));
		...
		try {
			...
			if (mandato.isAnnullato())
				...
				if (totRitenuteSuRigheMandato.compareTo(BigDecimal.ZERO)==0 && mandato.getIm_ritenute().compareTo(BigDecimal.ZERO)!=0) {
					...
					BigDecimal revCollegateNettoManuali = BigDecimal.ZERO;
					//trovo eventuali reversali sul documento generico. Possono essere state collegate direttamente al mandato
					List<Ass_mandato_reversaleBulk> result = ((Ass_mandato_reversaleHome) getHome(userContext, Ass_mandato_reversaleBulk.class)).findReversali(userContext, mandato, false);
					for (Ass_mandato_reversaleBulk assManRev : result) {
						ReversaleIHome revHome = (ReversaleIHome) getHome(userContext, ReversaleIBulk.class);
						ReversaleBulk reversale = (ReversaleIBulk) revHome.findByPrimaryKey(new ReversaleIBulk(assManRev.getCd_cds_reversale(), assManRev.getEsercizio_reversale(), assManRev.getPg_reversale()));
						...
						//Se reversale su documento generico entrata allora ipotizzo associazione manuale
						if (reversale.getReversale_rigaColl().stream()
								.allMatch(el->Documento_genericoBulk.GENERICO_E.equalsIgnoreCase(el.getCd_tipo_documento_amm()))) {
							TestataPrimaNota testataPrimaNotaReversale = this.proposeTestataPrimaNotaReversale(userContext, reversale, Boolean.FALSE);
							//Aggiungo i dettagli della reversale al mandato
							testataPrimaNota.getDett().addAll(testataPrimaNotaReversale.getDett());
						} else {
							revCollegateNettoManuali = revCollegateNettoManuali.add(reversale.getIm_reversale());
						}
					}
					if (totRitenuteSuRigheMandato.compareTo(revCollegateNettoManuali)!=0) {
		...
		try {
			...
			for (Tipo_sezionaleBulk tipoSezionale : tipiSezionale) {
				...
				for (IDocumentoAmministrativoBulk docamm : allDocumentiUoLiquidati.stream().filter(el->!el.isAnnullato()).collect(Collectors.toList())) {
					...
					List<Scrittura_partita_doppiaBulk> scrittureList;

					//recupero tutti i movimenti della partita per ottenere il saldo al netto della scrittura del mandato se già esiste
					if (Optional.of(docamm).filter(Fattura_passivaBulk.class::isInstance)
							.map(Fattura_passivaBulk.class::cast)
							.map(Fattura_passivaBulk::isGenerataDaCompenso).orElse(Boolean.FALSE))
						// Le fatture generate da compenso non creano scritture di prima nota in quanto create direttamente dal compenso stesso
						scrittureList = this.getScritturePartitaDoppia(userContext, ((Fattura_passivaBulk) docamm).getCompenso());
					else
						scrittureList = this.getScritturePartitaDoppia(userContext, docamm);

					if (scrittureList.isEmpty())
						throw new ApplicationRuntimeException("Errore nella generazione scrittura prima nota del tipo liquidazione IVA " + liqIvaUo.getTipo_liquidazione() +
								" della UO " + liqIvaUo.getCd_unita_organizzativa() + " per il periodo " + new SimpleDateFormat("dd/MM/yyyy").format(liqIvaUo.getDt_inizio()) +
					...
					*/
					...
					for (Scrittura_partita_doppiaBulk scrittura : scrittureList) {
						//Registro solo le scritture ante liquidazione
						if (!scrittura.getDt_contabilizzazione().after(liqIvaUo.getDacr())) {
							scrittura.getAllMovimentiColl()
									.stream()
			...
			BigDecimal saldoDebitoUo = testataPrimaNota.getDett().stream()
					...
					.filter(el -> Optional.ofNullable(el.getPartita()).isEmpty())
...
//				System.out.println("Liquidazione completata della UO: " + liquid.getCd_uo_origine());
	...
	private void addDettagliPrimaNotaMandatoDocumentiVari(UserContext userContext, TestataPrimaNota testataPrimaNota, MandatoBulk mandato, IDocumentoAmministrativoBulk docamm, Integer cdTerzoDocAmm, List<MandatoRigaComplete> mandatoRigaCompleteList) throws ComponentException, PersistencyException, RemoteException {
							...
							for (FatturaOrdineBulk fattordine : listaFatturaOrdini) {
								el.getDocammRighe().stream().filter(Fattura_passiva_rigaBulk.class::isInstance).map(Fattura_passiva_rigaBulk.class::cast)
										.filter(riga->riga.equalsByPrimaryKey(fattordine.getFatturaPassivaRiga())).findAny().ifPresent(rigaFattura -> {
											...
											fattordine.setOrdineAcqConsegna((OrdineAcqConsegnaBulk) loadObject(userContext, fattordine.getOrdineAcqConsegna()));
											list.add(new DettaglioFinanziario(docamm, rigaFattura, cdTerzoDocAmm, fattordine.getOrdineAcqConsegna().getContoBulk(), rigaFattura.getIm_imponibile(), rigaFattura.getIm_iva()));
										});
							}
							...
							el.getDocammRighe().stream().filter(Fattura_passiva_rigaBulk.class::isInstance).map(Fattura_passiva_rigaBulk.class::cast)
									.forEach(riga->{
										if (listaFatturaOrdini.stream().noneMatch(fatord-> fatord.getFatturaPassivaRiga().equalsByPrimaryKey(riga))) {
											...
											if (imponibile.compareTo(BigDecimal.ZERO)!=0||imposta.compareTo(BigDecimal.ZERO)!=0) {
												...
												list.add(new DettaglioFinanziario(docamm, riga, cdTerzoDocAmm, riga.getVoce_ep(), imponibile, imposta, dettagliAnalitici));
											}
										}
									});
		...
		if (imNettoRigheMandato.compareTo(BigDecimal.ZERO)>0)
			for (String cdVocePatrimoniale:contiPatrimonialiDaChiudere.keySet()) {
				...
				if (imponibileConto.compareTo(BigDecimal.ZERO)!=0) {
	...
	private Voce_epBulk findContoUtileCambi(UserContext userContext, Integer esercizio) throws ComponentException, RemoteException {
		return this.findContoByConfigurazioneCNR(userContext, esercizio, Configurazione_cnrBulk.PK_VOCEEP_SPECIALE, Configurazione_cnrBulk.SK_UTILE_SU_CAMBIO, 1);
	}
	...
	private Voce_epBulk findContoPerditeCambi(UserContext userContext, Integer esercizio) throws ComponentException, RemoteException {
		return this.findContoByConfigurazioneCNR(userContext, esercizio, Configurazione_cnrBulk.PK_VOCEEP_SPECIALE, Configurazione_cnrBulk.SK_PERDITA_SU_CAMBIO, 1);
	}
	...
	private Pair<Voce_epBulk, Voce_epBulk> findPairCosto(UserContext userContext, DettaglioFinanziario dettaglioFinanziario) throws ComponentException, RemoteException, PersistencyException {
		Voce_epBulk voceEp = dettaglioFinanziario.getVoceEp();
		if (voceEp==null) {
			Pair<ContoBulk, List<IDocumentoDetailAnaCogeBulk>> datiEcoDefault = loadDatiEcoDefault(userContext, dettaglioFinanziario.getRigaDocamm());
			voceEp = datiEcoDefault.getFirst();
		}
		...
		if (cdCausaleContabile!=null) {
			...
			if (!assCausaleVoce.isEmpty()) {
				...
				if (myContoContropartita.get()==null)
					throw new ApplicationException("Errore: non è stato possibile individuare per la causale contabile "+cdCausaleContabile+" un conto associato alla sezione "+mySezione);
		...
		Pair<Voce_epBulk, Voce_epBulk> pairConti = this.findPairCosto(userContext, new TerzoBulk(dettaglioFinanziario.getCdTerzo()), aContoCosto,
				dettaglioFinanziario.getDocamm().getTipoDocumentoEnum().getTipoPatrimoniale());

		boolean isDocumentoInContoAcconto = Optional.ofNullable(dettaglioFinanziario.getDocamm())
					.filter(Documento_genericoBulk.class::isInstance)
					.map(Documento_genericoBulk.class::cast)
					.map(Documento_genericoBulk::isDocumentoInContoAcconto)
					.orElse(Boolean.FALSE) ||
				Optional.ofNullable(dettaglioFinanziario.getDocamm())
					.filter(Fattura_attivaBulk.class::isInstance)
					.map(Fattura_attivaBulk.class::cast)
					.map(Fattura_attivaBulk::isDocumentoInContoAcconto)
					.orElse(Boolean.FALSE);
		boolean isDocumentoInContoAnticipo = Optional.ofNullable(dettaglioFinanziario.getDocamm())
					.filter(Documento_genericoBulk.class::isInstance)
					.map(Documento_genericoBulk.class::cast)
				...
				Optional.ofNullable(dettaglioFinanziario.getDocamm())
					.filter(Fattura_attivaBulk.class::isInstance)
					.map(Fattura_attivaBulk.class::cast)
					.map(Fattura_attivaBulk::isDocumentoInContoAnticipo)
					.orElse(Boolean.FALSE);

		if (isDocumentoInContoAcconto || isDocumentoInContoAnticipo) {
			Voce_epBulk aContoDefault = null;
			if (isDocumentoInContoAcconto)
				aContoDefault = ((Configurazione_cnrHome) getHome(userContext, Configurazione_cnrBulk.class))
						.getContoAccontoDocumentoAttivo(dettaglioFinanziario.getDocamm().getEsercizio());
			else aContoDefault = ((Configurazione_cnrHome) getHome(userContext, Configurazione_cnrBulk.class))
                    .getContoAnticipoDocumentoAttivo(dettaglioFinanziario.getDocamm().getEsercizio());
			if (aContoDefault != null) {
				aContoDefault = (Voce_epBulk) loadObject(userContext, aContoDefault);
				return Pair.of(aContoDefault, pairConti.getSecond());
			}
		}
		return pairConti;
	}
	...
	private List<Movimento_cogeBulk> findMovimentiPrimaNota(UserContext userContext, IDocumentoCogeBulk docamm) throws ComponentException, PersistencyException {
		...
		Integer esercizioScritture = this.getEsercizioScritture(userContext, docamm);
		...
		//L'economica è attiva per gli ordini anche se imputato correttamente il valore corrispondente

		boolean isAttivaEconomica = ((Configurazione_cnrHome)getHome(userContext, Configurazione_cnrBulk.class)).isAttivaEconomica(esercizioScritture) &&
				(!docamm.getTipoDocumentoEnum().isConsegnaOrdineAcquisto() ||
					((Configurazione_cnrHome)getHome(userContext, Configurazione_cnrBulk.class)).isAttivaEconomicaSuOrdini(esercizioScritture));
...
*/
	...
	private void addMovimentoCoan(UserContext userContext, Scrittura_analiticaBulk scritturaAnalitica, IDocumentoCogeBulk doccoge, Movimento_cogeBulk movimentoCoge, String aSezione, String aCdContoAna, String aCdCentroCosto, String aCdLineaAttivita, BigDecimal aImporto) throws ComponentException{
		try {
			...
			WorkpackageBulk lineaAttivita = ((WorkpackageHome)getHome(userContext, WorkpackageBulk.class)).searchGAECompleta(userContext,CNRUserContext.getEsercizio(userContext), aCdCentroCosto, aCdLineaAttivita, Boolean.FALSE);
	...
	private List<Scrittura_partita_doppiaBulk> getScritturePartitaDoppia(UserContext userContext, IDocumentoCogeBulk docamm) throws ComponentException, PersistencyException {
		return getScritturePartitaDoppia(userContext, docamm, Boolean.FALSE);
	}
	...
	private List<Scrittura_partita_doppiaBulk> getScritturePartitaDoppia(UserContext userContext, IDocumentoCogeBulk docamm, boolean createIfNotExist) throws ComponentException, PersistencyException {
		List<Scrittura_partita_doppiaBulk> scritture = new ArrayList<>();

		boolean isAttivaEconomicaDocamm = ((Configurazione_cnrHome) getHome(userContext, Configurazione_cnrBulk.class)).isAttivaEconomica(docamm.getEsercizio());
		if (isAttivaEconomicaDocamm)
			scritture = ((Scrittura_partita_doppiaHome) getHome(userContext, Scrittura_partita_doppiaBulk.class)).getScritture(userContext, docamm, Boolean.FALSE);
		...
		if (!isAttivaEconomicaDocamm || (createIfNotExist && scritture.isEmpty())) {
			...
			try {
				...
				if (scritturaOpt.isPresent())
					scritture.add(scritturaOpt.get());
			} catch (ScritturaPartitaDoppiaNotRequiredException | ScritturaPartitaDoppiaNotEnabledException ignored) {
			} catch (ComponentException e) {
				throw new ApplicationRuntimeException(e);
			}
	...
	private DettaglioFinanziario convertToRigaDettFin(UserContext userContext, IDocumentoDetailEcoCogeBulk rigaDocAmm) throws ComponentException, PersistencyException, RemoteException {
		...
		Pair<ContoBulk, List<IDocumentoDetailAnaCogeBulk>> datiEco;
		//Se non trovo caricati i dati allora prendo quelli di default senza salvarli nelle tabelle
		if (rigaDocAmm.getVoce_ep()==null || rigaDocAmm.getVoce_ep().getCd_voce_ep()==null)
			datiEco = this.loadDatiEcoDefault(userContext, rigaDocAmm);
		else
			datiEco = Pair.of(rigaDocAmm.getVoce_ep(), rigaDocAmm.getChildrenAna());

		for (IDocumentoDetailAnaCogeBulk rigaAna : datiEco.getSecond())
			dettagliAnalitici.add(new DettaglioAnalitico(rigaAna));

		if (datiEco.getFirst().getCd_voce_ep() == null)
			throw new ApplicationException("Riga documento senza indicazione del conto di economica. Scrittura economica non possibile.");

		return new DettaglioFinanziario(rigaDocAmm, rigaPartita, terzo.getCd_terzo(), datiEco.getFirst(), dettagliAnalitici);
	}
	...
	private Pair<ContoBulk, List<IDocumentoDetailAnaCogeBulk>> loadDatiEcoDefault(UserContext userContext, IDocumentoDetailEcoCogeBulk rigaEco) throws ComponentException, PersistencyException, RemoteException {
	...
	private void loadDocammRigheEcoFromCofi(UserContext userContext, IDocumentoDetailEcoCogeBulk rigaEco) throws ComponentException, PersistencyException, RemoteException {
		Pair<ContoBulk,List<IDocumentoDetailAnaCogeBulk>> datiEcoDefault = loadDatiEcoDefault(userContext,rigaEco);
...
//			getHome(userContext, (OggettoBulk) rigaEco, "UPDATE_ONLY_VOCEEP")
//					.update((Persistent) rigaEco, userContext);
