...
import it.cnr.contab.coepcoan00.core.bulk.Movimento_coanBulk;
...
public class AnaliticaDetailCRUDController extends CollapsableDetailCRUDController implements TableCustomizer {
    public AnaliticaDetailCRUDController(FormController formcontroller) {
        this(
                "Movimenti Analitici",
                it.cnr.contab.coepcoan00.core.bulk.Movimento_coanBulk.class,
                "scrittura_analitica.movimentiColl",
                formcontroller
        );
    }
    public AnaliticaDetailCRUDController(String s, Class class1, String s1, FormController formcontroller) {
        super(s, class1, s1, formcontroller);
        this.setCollapsed(Boolean.FALSE);
    }

    public AnaliticaDetailCRUDController(String s, Class class1, String s1, FormController formcontroller, boolean flag) {
    ...
    public void writeTfoot(JspWriter jspWriter) throws IOException {
        ...
        final List<Movimento_coanBulk> movimento_coanBulks = getDetails();
        if (Optional.ofNullable(movimento_coanBulks).map(movimento_coanBulks1 -> !movimento_coanBulks.isEmpty()).orElse(Boolean.FALSE) ) {
            final BigDecimal totalMovimento = movimento_coanBulks.stream().map(Movimento_coanBulk::getIm_movimento)
