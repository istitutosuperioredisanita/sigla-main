...
public class Anticipo_riga_ecoKey extends OggettoBulk implements KeyedPersistent {
    ...
    // PG_ANTICIPO DECIMAL(10,0) NOT NULL (PK)
    private Long pg_anticipo;

    // PROGRESSIVO_RIGA_ECO DECIMAL(10,0) NOT NULL (PK)
    private Long progressivo_riga_eco;

    public Anticipo_riga_ecoKey() {
        super();
    }

    public Anticipo_riga_ecoKey(String cd_cds, String cd_unita_organizzativa, Integer esercizio, Long pg_anticipo, Long progressivo_riga_eco) {
        this.cd_cds = cd_cds;
    ...
    public boolean equalsByPrimaryKey(Object o) {
        ...
        if (!(o instanceof Anticipo_riga_ecoKey)) return false;
        Anticipo_riga_ecoKey k = (Anticipo_riga_ecoKey) o;
        if (!compareKey(getCd_cds(), k.getCd_cds())) return false;
        if (!compareKey(getCd_unita_organizzativa(), k.getCd_unita_organizzativa())) return false;
        if (!compareKey(getEsercizio(), k.getEsercizio())) return false;
        if (!compareKey(getPg_anticipo(), k.getPg_anticipo())) return false;
    ...
    public int primaryKeyHashCode() {
        return
                calculateKeyHashCode(getCd_cds()) +
                        ...
                        calculateKeyHashCode(getPg_anticipo()) +
