 ...
 */
public class ConsPDGPTitComponent extends CRUDComponent {
		...
		private RemoteIterator findConsultazioneDettaglio(UserContext userContext, String pathDestinazione, String livelloDestinazione, CompoundFindClause baseClause, CompoundFindClause findClause, String tipoCons) throws it.cnr.jada.comp.ComponentException {
			try{
				...
				BulkHome home = getHome(userContext, V_cons_pdgp_titBulk.class, pathDestinazione);
			   ...
			   if (!((CNRUserContext) userContext).getCd_unita_organizzativa().equals( ente.getCd_unita_organizzativa())){			
				   ...
				   if (pathDestinazione.indexOf(ConsPDGPTitBP.LIVELLO_CDR)>=0)
				...
				return iterator(userContext,completaSQL(sql,sqlEsterna,tabAlias,baseClause,findClause),V_cons_pdgp_titBulk.class,null);
	...
	private void addBaseColumns(UserContext context,SQLBuilder sql, SQLBuilder sqlEsterna, String tabAlias, String pathDestinazione, String livelloDestinazione) throws it.cnr.jada.comp.ComponentException {
			...
			if (pathDestinazione.indexOf(ConsPDGPTitBP.LIVELLO_LIV1)>=0){	
				addColumnLIV1(sql,tabAlias,(livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_ETRLIV1)||livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_SPELIV1)),true,pathDestinazione,context);
				addColumnLIV1(sqlEsterna,tabAlias,(livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_ETRLIV1)||livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_SPELIV1)),false,pathDestinazione,context);
			}	
			if (pathDestinazione.indexOf(ConsPDGPTitBP.LIVELLO_CDR)>=0){
				addColumnCDR(sql,tabAlias,livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_CDR),true,pathDestinazione,context);
				addColumnCDR(sqlEsterna,tabAlias,livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_CDR),false,pathDestinazione,context);
			}			
			if (pathDestinazione.indexOf(ConsPDGPTitBP.LIVELLO_LIV2)>=0){
				addColumnLIV2(sql,tabAlias,livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_LIV2),true);
				addColumnLIV2(sqlEsterna,tabAlias,livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_LIV2),false);
			}
			if (pathDestinazione.indexOf(ConsPDGPTitBP.LIVELLO_LIV3)>=0){
				addColumnLIV3(sql,tabAlias,livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_LIV3),true);
				addColumnLIV3(sqlEsterna,tabAlias,livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_LIV3),false);
			}
			if (pathDestinazione.indexOf(ConsPDGPTitBP.LIVELLO_DET)>=0){
				addColumnDET(sql,tabAlias,livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_DET),true,pathDestinazione);
				addColumnDET(sqlEsterna,tabAlias,livelloDestinazione.equals(ConsPDGPTitBP.LIVELLO_DET),false,pathDestinazione);
			}	
	}			
		 ...
		 */
		private void addColumnCDR(SQLBuilder sql, String tabAlias, boolean addDescrizione, boolean isBaseSQL,String livello_arrivo,UserContext context) throws it.cnr.jada.comp.ComponentException{ 
			tabAlias = getAlias(tabAlias);
			...
			if (tabAlias.indexOf("V_CONS_PDG_SPE_BIL_IST_DIP_FO")>=0 && livello_arrivo.endsWith(ConsPDGPTitBP.LIVELLO_CDR)){			
					if (isBaseSQL){
						BulkHome home;
		...
		private void addColumnLIV1(SQLBuilder sql, String tabAlias, boolean addDescrizione, boolean isBaseSQL,String livello_arrivo,UserContext context) throws it.cnr.jada.comp.ComponentException{ 
			...
			if (tabAlias.indexOf("V_CONS_PDG_SPE_BIL_IST_DIP_FO")>=0 && livello_arrivo.compareTo(ConsPDGPTitBP.LIVELLO_SPELIV1)==0){
				if (isBaseSQL){
					BulkHome home;
					home = getHome(context, Pdg_modulo_costiBulk.class);
					SQLBuilder sqlsum = home.createSQLBuilder();
	...
	/**
			 * Aggiunge nell'SQLBuilder <sql> le colonne del terzo livello della classificazione.
			 *
			 * @param sql l'SQLBuilder da aggiornare
			 * @param tabAlias l'alias della tabella da aggiungere alle colonne interrogate
			 * @param addDescrizione se TRUE aggiunge anche la colonna della Descrizione
