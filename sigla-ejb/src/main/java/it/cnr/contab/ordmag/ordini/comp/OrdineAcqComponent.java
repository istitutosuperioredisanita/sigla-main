...
public class OrdineAcqComponent
    ...
    protected void impostaCampiDspRiga(UserContext userContext, OrdineAcqRigaBulk riga) throws ComponentException {
        try {
            ...
            if (riga.getRigheConsegnaColl().size() == 1) {
                ...
                if (!Utility.createConfigurazioneCnrComponentSession().isAttivaEconomicaPura(userContext, riga.getEsercizio())) {
    ...
    protected Boolean isCdrUo(UserContext userContext) throws ComponentException, PersistencyException {
        ...
        sqlStr.addSQLClause(FindClause.AND, "V_STRUTTURA_ORGANIZZATIVA.ESERCIZIO", SQLBuilder.EQUALS, CNRUserContext.getEsercizio(userContext));
        sqlStr.addSQLClause(FindClause.AND, "V_STRUTTURA_ORGANIZZATIVA.CD_CDS", SQLBuilder.EQUALS, CNRUserContext.getCd_cds(userContext));
        sqlStr.addSQLClause(FindClause.AND, "V_STRUTTURA_ORGANIZZATIVA.CD_UNITA_ORGANIZZATIVA", SQLBuilder.EQUALS, CNRUserContext.getCd_unita_organizzativa(userContext));
        sqlStr.addSQLClause(FindClause.AND, "V_STRUTTURA_ORGANIZZATIVA.CD_CENTRO_RESPONSABILITA", SQLBuilder.EQUALS, CNRUserContext.getCd_cdr(userContext));
        sqlStr.addSQLClause(FindClause.AND, "V_STRUTTURA_ORGANIZZATIVA.CD_TIPO_LIVELLO", SQLBuilder.EQUALS, V_struttura_organizzativaHome.LIVELLO_CDR);
        sqlStr.addSQLClause(FindClause.AND, "V_STRUTTURA_ORGANIZZATIVA.FL_CDR_UO", SQLBuilder.EQUALS, "Y");
    ...
    public SQLBuilder selectTerzoCdrByClause(UserContext userContext, OggettoBulk bulk, TerzoBulk terzo, CompoundFindClause clause) throws ComponentException, PersistencyException {
        ...
        sql.addSQLClause("AND", "CD_UNITA_ORGANIZZATIVA", SQLBuilder.ISNOTNULL, null);
...
*/
    ...
    private it.cnr.jada.persistency.sql.SQLBuilder ricercaObbligazioni(UserContext context,
        ...
        if (filtro.getListaVociSelezionabili() != null && !filtro.getListaVociSelezionabili().isEmpty()) {
            ...
            for (Elemento_voceBulk voce : filtro.getListaVociSelezionabili()) {
                ...
                sql.addSQLClause("AND", "OBBLIGAZIONE.CD_ELEMENTO_VOCE", SQLBuilder.EQUALS, voce.getCd_elemento_voce());
                sql.addSQLClause("AND", "OBBLIGAZIONE.TI_APPARTENENZA", SQLBuilder.EQUALS, voce.getTi_appartenenza());
                sql.addSQLClause("AND", "OBBLIGAZIONE.TI_GESTIONE", SQLBuilder.EQUALS, voce.getTi_gestione());
                sql.addSQLClause("AND", "OBBLIGAZIONE.ESERCIZIO", SQLBuilder.EQUALS, voce.getEsercizio());
                sql.closeParenthesis();
        ...
        if (filtro.getContratto() != null && filtro.getContratto().getPg_contratto() != null) {
            sql.addSQLClause("AND", "OBBLIGAZIONE.PG_CONTRATTO", SQLBuilder.EQUALS, filtro.getContratto().getPg_contratto());
            sql.addSQLClause("AND", "OBBLIGAZIONE.ESERCIZIO_CONTRATTO", SQLBuilder.EQUALS, filtro.getContratto().getEsercizio());
            sql.addSQLClause("AND", "OBBLIGAZIONE.STATO_CONTRATTO", SQLBuilder.EQUALS, filtro.getContratto().getStato());
        }
    ...
    private List<Obbligazione_scadenzarioBulk> recuperoScadenzaLibera(UserContext userContext, Obbligazione_scadenzarioBulk scadenza) throws ComponentException, PersistencyException {
        ...
        sql.addClause("AND", "cdCds", SQLBuilder.EQUALS, scadenza.getCd_cds());
        sql.addClause("AND", "esercizio", SQLBuilder.EQUALS, scadenza.getEsercizio());
        sql.addClause("AND", "esercizio_originale", SQLBuilder.EQUALS, scadenza.getEsercizio_originale());
        sql.addClause("AND", "pg_obbligazione", SQLBuilder.EQUALS, scadenza.getPg_obbligazione());
    ...
    private Boolean isImpegnoCollegatoInteramenteOrdine(UserContext userContex, OrdineAcqBulk ordine, Obbligazione_scadenzarioBulk obbligazione_scadenzario) throws ComponentException, PersistencyException {
        ...
        sql.addClause("AND", "esercizioObbl", SQLBuilder.EQUALS, obbligazione_scadenzario.getEsercizio());
...
// Serve a capire se l'impegno è usato solo per quest'ordine o anche per altri ordini. Se è usato anche per altri non riduco l'impegno a prescindere, ma riduco solo l'importo collegato all'ordine.
    ...
    public SQLBuilder selectContoBulkByClause(UserContext userContext, OrdineAcqConsegnaBulk bulk, ContoBulk contoBulk, CompoundFindClause clauses) throws PersistencyException, ComponentException {
        ...
        SQLBuilder sql = home.createSQLBuilder();
        OrdineAcqConsegnaBulk consegna = bulk;
        try {
    ...
    public SQLBuilder selectVoce_analitica_voce_epByClause(UserContext userContext, OrdineAcqEcoBulk ordineAcqEcoBulk, ContoBulk contoBulk, CompoundFindClause clause)  throws ComponentException, EJBException, RemoteException {
        ContoHome contoHome = (ContoHome) getHome(userContext, ContoBulk.class);
        SQLBuilder sql = contoHome.createSQLBuilder();
        sql.addSQLClause(FindClause.AND,"ESERCIZIO",SQLBuilder.EQUALS, CNRUserContext.getEsercizio(userContext));
        sql.addClause(clause);
        ...
        try {
            for (OrdineAcqRigaBulk rigaOrdine:ordine.getRigheOrdineColl()) {
                for (OrdineAcqConsegnaBulk consegnaOrdine:rigaOrdine.getRigheConsegnaColl()) {
                    ...
                    //quindi inserisco
                    if (consegnaOrdine.getImCostoEco().compareTo(BigDecimal.ZERO)>0 && rigaOrdine.getImCostoEco().compareTo(BigDecimal.ZERO)>0) {
                        for (OrdineAcqRigaEcoBulk rigaEco : rigaOrdine.getRigheEconomica()) {
                            ...
                            consEco.setOrdineAcqConsegna(consegnaOrdine);
                            consEco.setVoce_analitica(rigaEco.getVoce_analitica());
                            consEco.setLinea_attivita(rigaEco.getLinea_attivita());
                            consEco.setImporto(rigaEco.getImporto().multiply(consegnaOrdine.getImCostoEco())
                                    .divide(rigaOrdine.getImCostoEco(), 2, RoundingMode.HALF_UP));
                            consEco.setToBeCreated();
                            consegnaOrdine.getRigheEconomica().add(consEco);
                        }
                        BigDecimal diff = consegnaOrdine.getImCostoEcoDaRipartire();
                        if (diff.compareTo(BigDecimal.ZERO) > 0) {
                            for (OrdineAcqConsegnaEcoBulk rigaEco : consegnaOrdine.getRigheEconomica()) {
