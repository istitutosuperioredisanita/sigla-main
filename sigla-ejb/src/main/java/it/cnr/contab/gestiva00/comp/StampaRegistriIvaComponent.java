        ...
        if (stampaBulk.getTipo_stampa().equals("GESTIONE")) {
            ...
            try {
                try {
                    ...
                    if (stampaBulk.getTipo_stampa().equals(Stampa_registri_ivaVBulk.TIPO_STAMPA_LIQUIDAZIONE) && stampaBulk.getTipo_report().equals(Stampa_registri_ivaVBulk.DEFINITIVO))
                    ...
                    } else if (stampaBulk.getTipo_stampa() != null && stampaBulk.getTipo_stampa().equals(Stampa_registri_ivaVBulk.TIPO_STAMPA_RIEPILOGATIVI_IVA_DIFFERITA))
                        cs.setString(8, ((Riepilogativi_ivaVBulk) stampaBulk).getSezionaliFlag());
                    else
                    ...
                    cs.setString(10, ((stampaBulk.isRistampa()) ?
                            Stampa_registri_ivaVBulk.RISTAMPA :
                            Stampa_registri_ivaVBulk.NON_RISTAMPA)); // Ristampa = 'N' [no ristampa] / 'Y' [ristampa]
            ...
            if (stampaBulk.isLiquidazione()) {
                try {
                    ...
                    sql.addClause("AND", "cd_cds", SQLBuilder.EQUALS, liquidazione.getCd_cds());
                    sql.addClause("AND", "esercizio", SQLBuilder.EQUALS, liquidazione.getEsercizio());
                    sql.addClause("AND", "cd_unita_organizzativa", SQLBuilder.EQUALS, liquidazione.getCd_unita_organizzativa());
...
//^^@@
    private void controllaSezionali(it.cnr.jada.UserContext userContext, Stampa_registri_ivaVBulk stampaBulk) throws it.cnr.jada.comp.ComponentException {
        ...
        try {
            ...
            sql.addSQLClause("AND", "STATO", SQLBuilder.EQUALS, "B");
            sql.addSQLClause("AND", "DT_INIZIO", SQLBuilder.EQUALS, new Date(stampaBulk.getData_da().getTime()));
            sql.addSQLClause("AND", "DT_FINE", SQLBuilder.EQUALS, new Date(stampaBulk.getData_a().getTime()));
            sql.addSQLClause("AND", "ESERCIZIO", SQLBuilder.EQUALS, bulk.getEsercizio());
            sql.addSQLClause("AND", "CD_CDS", SQLBuilder.EQUALS, bulk.getUnita_organizzativa().getUnita_padre().getCd_unita_organizzativa());
            sql.addSQLClause("AND", "CD_UNITA_ORGANIZZATIVA", SQLBuilder.EQUALS, bulk.getUnita_organizzativa().getCd_unita_organizzativa());
            sql.addSQLClause("AND", "TIPO_REPORT", SQLBuilder.EQUALS, "REGISTRO_IVA");
            sql.addSQLClause("AND", "CD_TIPO_SEZIONALE", SQLBuilder.EQUALS, bulk.getTipo_sezionale().getCd_tipo_sezionale());
        ...
        if (bulk.getTipo_report().equals("ANNULLA")) {
            ...
            try {
                ...
                sql.addSQLClause("AND", "STATO", SQLBuilder.EQUALS, "D");
                sql.addSQLClause("AND", "REPORT_ID", SQLBuilder.EQUALS, "0");
                sql.addSQLClause("AND", "DT_INIZIO", SQLBuilder.EQUALS, new Date(stampaBulk.getData_da().getTime()));
                sql.addSQLClause("AND", "DT_FINE", SQLBuilder.EQUALS, new Date(stampaBulk.getData_a().getTime()));
                sql.addSQLClause("AND", "ESERCIZIO", SQLBuilder.EQUALS, bulk.getEsercizio());
                sql.addSQLClause("AND", "CD_CDS", SQLBuilder.EQUALS, bulk.getUnita_organizzativa().getUnita_padre().getCd_unita_organizzativa());
...
//^^@@
    public java.util.Collection selectProspetti_stampatiByClause(
        ...
        Scrittura_partita_doppiaHome scritturaPartitaDoppiaHome = (Scrittura_partita_doppiaHome) getHome(aUC, Scrittura_partita_doppiaBulk.class);
        it.cnr.jada.persistency.sql.SQLBuilder sql = home.createSQLBuilder();
        ...
        sql.addSQLClause("AND", "CD_CDS", SQLBuilder.EQUALS, stampaBulk.getCd_cds());
        sql.addSQLClause("AND", "CD_UNITA_ORGANIZZATIVA", SQLBuilder.EQUALS, stampaBulk.getCd_unita_organizzativa());
        sql.addSQLClause("AND", "ESERCIZIO", SQLBuilder.EQUALS, stampaBulk.getEsercizio());
        sql.addSQLClause("AND", "STATO", SQLBuilder.EQUALS, Stampa_registri_ivaVBulk.DEFINITIVO);
        sql.addSQLClause("AND", "TIPO_LIQUIDAZIONE", SQLBuilder.EQUALS, stampaBulk.getTipoSezionaleFlag());
        final List<Liquidazione_ivaBulk> bulks = home.fetchAll(sql);
        return         bulks.stream().map(liquidazioneIvaBulk -> {
            try {
                scritturaPartitaDoppiaHome.getScrittura(aUC, liquidazioneIvaBulk, Boolean.TRUE)
                        .ifPresent(scritturaPartitaDoppiaBulk -> liquidazioneIvaBulk.setScrittura_partita_doppia(scritturaPartitaDoppiaBulk));
            } catch (ComponentException e) {
                throw new RuntimeException(e);
            }
            return liquidazioneIvaBulk;
...
//^^@@
    public java.util.Collection selectTipi_sezionaliByClause(
        ...
        sql.addSQLClause("AND", "SEZIONALE.CD_CDS", SQLBuilder.EQUALS, stampaBulk.getCd_cds());
        sql.addSQLClause("AND", "SEZIONALE.CD_UNITA_ORGANIZZATIVA", SQLBuilder.EQUALS, stampaBulk.getCd_unita_organizzativa());
        sql.addSQLClause("AND", "SEZIONALE.ESERCIZIO", SQLBuilder.EQUALS, stampaBulk.getEsercizio());

        if (stampaBulk.getTipoSezionaleFlag() != null) {
            if (Stampa_registri_ivaVBulk.SEZIONALI_COMMERCIALI.equalsIgnoreCase(stampaBulk.getTipoSezionaleFlag()))
                sql.addSQLClause("AND", "TIPO_SEZIONALE.TI_ISTITUZ_COMMERC", SQLBuilder.EQUALS, TipoIVA.COMMERCIALE.value());
            else if (Stampa_registri_ivaVBulk.SEZIONALI_SAN_MARINO_SENZA_IVA.equalsIgnoreCase(stampaBulk.getTipoSezionaleFlag())) {
                sql.addSQLClause("AND", "TIPO_SEZIONALE.FL_SAN_MARINO_SENZA_IVA", SQLBuilder.EQUALS, "Y");
                sql.addSQLClause("AND", "TIPO_SEZIONALE.TI_ISTITUZ_COMMERC", SQLBuilder.EQUALS, TipoIVA.ISTITUZIONALE.value());
                sql.addSQLClause("AND", "TIPO_SEZIONALE.TI_BENE_SERVIZIO", SQLBuilder.NOT_EQUALS, Bene_servizioBulk.SERVIZIO);
		/* RP INTRASTAT EVENTUALMENTE UTILE SE RICHIESTO UN'UNICA LIQUIDAZIONE
            ...
            } else if (Stampa_registri_ivaVBulk.SEZIONALI_BENI_INTRA_UE.equalsIgnoreCase(stampaBulk.getTipoSezionaleFlag())) {
                sql.addSQLClause("AND", "TIPO_SEZIONALE.FL_INTRA_UE", SQLBuilder.EQUALS, "Y");
                sql.addSQLClause("AND", "TIPO_SEZIONALE.TI_ISTITUZ_COMMERC", SQLBuilder.EQUALS, TipoIVA.ISTITUZIONALE.value());
                sql.addSQLClause("AND", "TIPO_SEZIONALE.TI_BENE_SERVIZIO", SQLBuilder.EQUALS, Bene_servizioBulk.BENE);
            } else if (Stampa_registri_ivaVBulk.SEZIONALI_SERVIZI_NON_RESIDENTI.equalsIgnoreCase(stampaBulk.getTipoSezionaleFlag())) {
                sql.openParenthesis("AND");
                sql.addSQLClause("AND", "TIPO_SEZIONALE.FL_INTRA_UE", SQLBuilder.EQUALS, "Y");
                sql.addSQLClause("OR", "TIPO_SEZIONALE.FL_EXTRA_UE", SQLBuilder.EQUALS, "Y");
                sql.addSQLClause("OR", "TIPO_SEZIONALE.FL_SAN_MARINO_SENZA_IVA", SQLBuilder.EQUALS, "Y");
                sql.closeParenthesis();
                sql.addSQLClause("AND", "TIPO_SEZIONALE.TI_ISTITUZ_COMMERC", SQLBuilder.EQUALS, TipoIVA.ISTITUZIONALE.value());
                sql.addSQLClause("AND", "TIPO_SEZIONALE.TI_BENE_SERVIZIO", SQLBuilder.EQUALS, Bene_servizioBulk.SERVIZIO);
            } else if (Stampa_registri_ivaVBulk.SEZIONALI_SPLIT_PAYMENT.equalsIgnoreCase(stampaBulk.getTipoSezionaleFlag())) {
...
//^^@@
    public it.cnr.jada.persistency.sql.SQLBuilder selectUnita_organizzativaByClause(
        ...
        if (((Gestione_registri_ivaVBulk) stampaBulk).getUnita_organizzativa() != null)
            sql.addSQLClause("AND", "CD_UNITA_ORGANIZZATIVA", SQLBuilder.EQUALS, ((Gestione_registri_ivaVBulk) stampaBulk).getUnita_organizzativa().getCd_unita_organizzativa());
        return sql;
    }
    ...
    public SQLBuilder selectLinea_di_attivitaByClause(UserContext userContext, Liquidazione_iva_ripart_finBulk ripartizione, WorkpackageBulk linea_di_attivita, CompoundFindClause clause) throws ComponentException, PersistencyException {
        ...
        try {
            if (it.cnr.jada.util.ejb.EJBCommonServices.createEJB("CNRCONFIG00_EJB_Parametri_cnrComponentSession", Parametri_cnrComponentSession.class).isCofogObbligatorio(userContext))
