...
//^^@@
    ...
    public AccertamentoBulk aggiornaScadenzarioSuccessivoAccertamento(UserContext aUC, Accertamento_scadenzarioBulk scadenza) throws ComponentException {
        ...
        if ((scadenza.getFl_aggiorna_scad_successiva() == null) || (!scadenza.getFl_aggiorna_scad_successiva().booleanValue())) {
            return accertamento;
        }
...
*/
            ...
            if (accertamento.getAccertamentiPluriennali() != null && !accertamento.getAccertamentiPluriennali().isEmpty()) {
                Accertamento_pluriennale_voceHome accertPlurVoceHome = (Accertamento_pluriennale_voceHome) getHome(aUC, Accertamento_pluriennale_voceBulk.class);

                for (Accertamento_pluriennaleBulk accPlur : accertamento.getAccertamentiPluriennali()) {
                    accPlur.setRigheVoceColl(new BulkList(accertPlurVoceHome.findAccertamentiPluriennaliVoce(aUC, accPlur)));
                }
            }
...
//			scadenza.getAccertamento().setIm_reversali( scadenza.getAccertamento().getIm_reversali().add( reversale.getIm_reversale_riga()));
    ...
    public OggettoBulk modificaConBulk(UserContext aUC, OggettoBulk bulk) throws ComponentException {
        try {
            ...
            creaAccertamentoPluriennaleVoce(aUC, (AccertamentoBulk) bulk, linee);
    ...
    private void creaAccertamentoPluriennaleVoce(UserContext aUC, AccertamentoBulk accertamento, HashMap<it.cnr.contab.config00.latt.bulk.WorkpackageBulk, BigDecimal> linee) {
        ...
        if (accertamento.getAccertamentiPluriennali() != null && !accertamento.getAccertamentiPluriennali().isEmpty()) {
            ...
            for (Accertamento_pluriennaleBulk accPlur : accertamento.getAccertamentiPluriennali()) {
                ...
                for (Accertamento_pluriennale_voceBulk accPlurVoce : accPlur.getRigheVoceColl()) {
                    accPlurVoce.setToBeDeleted();
                }
