...
public class CRUDObbligazioneAction extends CRUDAbstractObbligazioneAction {
    ...
    public Forward doBlankSearchFind_creditore(ActionContext context, ObbligazioneBulk obbligazione) {
        try {
            final Boolean isStudioAssociato = Optional.ofNullable(obbligazione)
     ...
     * @return <code>Forward</code>
     */
    public Forward doBringBackCRUDFind_creditore(ActionContext context, ObbligazioneBulk obbligazione, TerzoBulk terzo) {
        try {
            CRUDObbligazioneBP bp = (CRUDObbligazioneBP) context.getBusinessProcess();
            if (terzo != null) {
                it.cnr.jada.util.RemoteIterator ri = null;
                try {
                    ri = bp.createComponentSession().cerca(
                            context.getUserContext(),
                            null,
                            terzo, obbligazione, "creditore");
                    ri = it.cnr.jada.util.ejb.EJBCommonServices.openRemoteIterator(context, ri);
                    if (ri.countElements() == 0) {
                        throw new it.cnr.jada.comp.ApplicationException("Impossibile caricare il terzo selezionato!");
                    }
    ...
    public Forward doBringBackSearchFind_elemento_voce(ActionContext context, ObbligazioneBulk obbligazione, Elemento_voceBulk capitolo) {
        try {
            ...
            if (capitolo != null) {
                ...
                obbligazione.setEnableVoceNext(!((ObbligazioneComponentSession) bp.createComponentSession()).existAssElementoVoceNew(context.getUserContext(), obbligazione));
            }
    ...
    public Forward doBringBackObbligazioniModificaWindow(ActionContext context) {
        ...
        try {
            ...
            if (obbMod != null) {
				...
				obbligazione.setSaldiDaAggiornare(!IDefferedUpdateSaldiBP.class.isAssignableFrom(bp.getParent().getClass()));
                return super.doSalva(context);
            }
