 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class ListaSospesiBP extends SelezionatoreListaBP implements it.cnr.jada.util.action.SearchProvider{
	...
	public it.cnr.jada.util.RemoteIterator findFreeSearch(
		...
		try {
			ReversaleComponentSession session = (ReversaleComponentSession) createRicercaComponentSession();
			CRUDReversaleBP reversaleBP = (CRUDReversaleBP)getParent();
			return session.cercaSospesi( context.getUserContext(), clauses, (ReversaleBulk)reversaleBP.getModel());			
		} catch(Exception e) {
