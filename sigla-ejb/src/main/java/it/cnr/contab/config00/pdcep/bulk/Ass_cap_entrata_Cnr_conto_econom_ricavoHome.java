...
public Ass_cap_entrata_Cnr_conto_econom_ricavoHome(java.sql.Connection conn, it.cnr.jada.persistency.PersistentCache persistentCache) {
	...
	public void initializePrimaryKeyForInsert(it.cnr.jada.UserContext userContext, OggettoBulk bulk) throws PersistencyException, ComponentException {
		Ass_cap_entrata_Cnr_conto_econom_ricavoBulk ep= new Ass_cap_entrata_Cnr_conto_econom_ricavoBulk();
		Integer max = (Integer)findMax(ep, "pg_ass_ev_voceep", null);
		if (max!=null)
			((Ass_cap_entrata_Cnr_conto_econom_ricavoBulk)bulk).setPg_ass_ev_voceep(new Integer(max.intValue()+1));
		else
			((Ass_cap_entrata_Cnr_conto_econom_ricavoBulk)bulk).setPg_ass_ev_voceep(new Integer(1));
	}
/**
