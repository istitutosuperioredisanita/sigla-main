 ...
 * @author: Roberto Peli
 */
public class RicercaDocumentiAmministrativiFatturazioneElettronicaAction extends ListaDocumentiAmministrativiAction {
    /**
     * DocumentiAmministrativiProtocollabiliAction constructor comment.
     */
    public RicercaDocumentiAmministrativiFatturazioneElettronicaAction() {
        super();
    }
        ...
        try {
            ...
            if (filtro.isDaFirmare()) {
            	if (bp.isUtenteNonAbilitatoFirma(context.getUserContext())){
                    ...
                    clausesFir.addClause("OR", "statoInvioSdi", SQLBuilder.EQUALS, VDocammElettroniciAttiviBulk.FATT_ELETT_ALLA_FIRMA);
                	CompoundFindClause clausesPre = new CompoundFindClause();
                    clausesPre.addClause("OR", "statoInvioSdi", SQLBuilder.EQUALS, VDocammElettroniciAttiviBulk.FATT_ELETT_PREDISPOSTA_FIRMA);
...
//                nbp.setMultiSelection(false);
    ...
    public Forward doSearchSoggettowithformname(ActionContext context) {
        return doSearchSoggetto(context);
    }
