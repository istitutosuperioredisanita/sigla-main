...
public class CRUDSelezionatoreDocumentiAmministrativiFatturazioneElettronicaBP extends SelezionatoreListaBP implements SearchProvider {
    ...
    public void scaricaDocumentiCollegati(ActionContext actioncontext) throws Exception {
        ...
        if (Numerazione_doc_ammBulk.TIPO_FATTURA_ATTIVA.equals(tipoDocamm)) {
            docammElettronico = (Fattura_attivaBulk) Utility.createFatturaAttivaSingolaComponentSession().findByPrimaryKey(actioncontext.getUserContext(), new Documento_amministrativo_attivoBulk(cds, cdUo, esercizio, pgFattura));
            storageObject = documentiCollegatiDocAmmService.recuperoFolderFattura((Fattura_attivaBulk) docammElettronico);
        } else if (Numerazione_doc_ammBulk.TIPO_AUTOFATTURA.equals(tipoDocamm)) {
            docammElettronico = (AutofatturaBulk) Utility.createAutoFatturaComponentSession().findByPrimaryKey(actioncontext.getUserContext(), new AutofatturaBulk(cds, cdUo, esercizio, pgFattura));
            storageObject = documentiCollegatiDocAmmService.recuperoFolderAutofattura((AutofatturaBulk) docammElettronico);
        }

        InputStream is = null;
        if (storageObject == null) {
            is = getStreamNewDocument(actioncontext, docammElettronico);
        } else {
            //is = documentiCollegatiDocAmmService.getStreamDocumentoAttivo(esercizio, cds, cdUo, pgFattura);
            is = documentiCollegatiDocAmmService.getStreamDocumentoAttivo(docammElettronico);
            if (is == null) {
                is = getStreamNewDocument(actioncontext, docammElettronico);
            }
        }
    ...
    private InputStream getStreamNewDocument(ActionContext actioncontext,
                                             IDocumentoAmministrativoElettronicoBulk docammElettronico )
            throws Exception {
        InputStream is;
        UserContext userContext = actioncontext.getUserContext();
        SpringUtil.getBean("documentiCollegatiDocAmmService", DocumentiCollegatiDocAmmService.class).gestioneAllegatiPerFatturazioneElettronica(userContext, docammElettronico);
        //is = documentiCollegatiDocAmmService.getStreamDocumentoAttivo(esercizio, cds, cdUo, pgFattura);
        is = documentiCollegatiDocAmmService.getStreamDocumentoAttivo(docammElettronico);
        return is;
    }
