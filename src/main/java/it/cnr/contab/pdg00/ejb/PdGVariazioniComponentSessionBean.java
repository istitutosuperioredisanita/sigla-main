...
public class PdGVariazioniComponentSessionBean extends it.cnr.jada.ejb.CRUDComponentSessionBean implements PdGVariazioniComponentSession {
	...
	public OggettoBulk creaConBulk(UserContext usercontext, OggettoBulk oggettobulk, boolean sendMessage) throws ComponentException, EJBException {
		pre_component_invocation(usercontext, componentObj);
		try {
			OggettoBulk oggettobulk1 = ((PdGVariazioniComponent) componentObj).creaConBulk(usercontext, oggettobulk, sendMessage);
			component_invocation_succes(usercontext, componentObj);
			return oggettobulk1;
