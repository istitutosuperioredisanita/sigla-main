...
public class MovimentiMagComponent extends CalcolaImportiMagComponent implements ICRUDMgr, IPrintMgr,Cloneable, Serializable {
    ...
    private MovimentiMagBulk createMovimentoMagazzino(UserContext userContext, OrdineAcqConsegnaBulk consegna, 
		...
		try {
			...
			BigDecimal prezzoIvaUnitario = importo.getPrezzoIvaUnitaria();
...
//			tipoMovimentoAz = (TipoMovimentoMagAzBulk)home.findByPrimaryKey(tipoMovimentoAz);
	...
	public MovimentiMagBulk creaMovimentoRettificaValoreOrdine(UserContext userContext, OrdineAcqConsegnaBulk ordineAcqConsegnaBulk, ImportoOrdine importoCns, ImportoOrdine importoRet) throws ComponentException {
        ...
        {
			BigDecimal prezzoUnitarioOrdine = importoCns.getPrezzoUnitario();
			BigDecimal prezzoIvaUnitarioOrdine = importoCns.getPrezzoIvaUnitaria();

			BigDecimal prezzoUnitarioOrdineRettificato = importoRet.getPrezzoUnitario();
			BigDecimal prezzoUnitarioIvaOrdineRettificato = importoRet.getPrezzoIvaUnitaria();
	...
	private void impostaImportiCorrettiPerCarico(UserContext userContext,CaricoMagazzinoBulk carico) throws ComponentException, RemoteException, PersistencyException {
		...
		for(CaricoMagazzinoRigaBulk riga : carico.getCaricoMagazzinoRigaColl()){
			...
			riga.setImpIva(importi.getPrezzoIvaUnitaria());
