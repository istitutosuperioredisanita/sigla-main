...
CREATE OR REPLACE FORCE VIEW "V_INDICATORE_PAGAMENTI_DETAIL" (
    ...
    FROM (
        SELECT f.esercizio esercizio_documento,f.CD_UO_ORIGINE uo_documento, f.PG_FATTURA_PASSIVA numero_documento,
              ...
              TRUNC ((m.dt_emissione) - nvl(f.dt_scadenza,nvl(f.data_protocollo,f.dt_registrazione)+30) - (m.dt_emissione - nvl(f.dt_inizio_sospensione, m.dt_emissione))) DIFFERENZA_GIORNI,
              (mr.im_mandato_riga) importo_pagato,
              ((TRUNC (m.dt_emissione) - nvl(f.dt_scadenza,nvl(f.data_protocollo,f.dt_registrazione)+30) - (m.dt_emissione - nvl(f.dt_inizio_sospensione, m.dt_emissione)))* mr.im_mandato_riga) importo_pesato,
