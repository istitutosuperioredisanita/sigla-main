...
If INGEST = 'S' Then
   ...
   WHERE osv.ESERCIZIO = INES
   AND   osv.ESERCIZIO_ORIGINALE = Nvl(INESRES, osv.ESERCIZIO_ORIGINALE)
   AND   osv.TI_APPARTENENZA = INAPP
   AND   osv.TI_GESTIONE = INGEST
   AND   osv.CD_VOCE = INELVOCE
   AND   osv.CD_CENTRO_RESPONSABILITA = INCDR
   AND   osv.CD_LINEA_ATTIVITA = INLA
   AND   NVL(n.IM_VARIAZIONE,0) > 0;
...
Elsif INGEST = 'E' Then
   ...
   LEFT JOIN ACCERTAMENTO acc ON acc.CD_CDS = n.CD_CDS AND acc.ESERCIZIO = n.ESERCIZIO AND acc.ESERCIZIO_ORIGINALE = n.ESERCIZIO_ORIGINALE AND acc.PG_ACCERTAMENTO = n.PG_ACCERTAMENTO
   WHERE acc.ESERCIZIO = INES
   AND   acc.ESERCIZIO_ORIGINALE = Nvl(INESRES, acc.ESERCIZIO_ORIGINALE)
   AND   acc.TI_APPARTENENZA = INAPP
   AND   acc.TI_GESTIONE = INGEST
   AND   acc.CD_ELEMENTO_VOCE = Nvl(INELVOCE, acc.CD_ELEMENTO_VOCE)
   AND   asv.CD_CENTRO_RESPONSABILITA = INCDR
   AND   asv.CD_LINEA_ATTIVITA = INLA
   AND   NVL(n.IM_VARIAZIONE,0) > 0;
End If;
