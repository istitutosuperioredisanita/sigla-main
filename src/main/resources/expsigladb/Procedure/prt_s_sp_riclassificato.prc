...
-- Date: 27/05/2026
-- Version: 1.2
-- Lettura dati da BIL_RICLASSIFICATO (se presente) con fallback su
-- MOVIMENTO_COGE, allineato al comportamento di PRT_S_CE_RICLASSIFICATO_J.
...
BEGIN
            v_uo_ente := CNRCTB020.getuoente(INES).cd_unita_organizzativa;
BEGIN
SELECT DECODE(SEZIONE, 'D', IMPORTO_FINALE, 0),
       DECODE(SEZIONE, 'A', IMPORTO_FINALE, 0),
...
WHERE  ESERCIZIO              = INES
  AND  CD_VOCE_EP             = CONTI_ASS.CD_VOCE_EP
  AND  CD_TIPO_BILANCIO       = cdTipoBilancio
...
v_uo_ente VARCHAR2(30);
BEGIN
            v_uo_ente := CNRCTB020.getuoente(ANNO_PREC).cd_unita_organizzativa;
BEGIN
SELECT DECODE(SEZIONE, 'D', IMPORTO_FINALE, 0),
       DECODE(SEZIONE, 'A', IMPORTO_FINALE, 0)
INTO   DARE2, AVERE2
FROM   BIL_RICLASSIFICATO
WHERE  ESERCIZIO              = ANNO_PREC
  AND  CD_VOCE_EP             = CONTI_ASS.CD_VOCE_EP
  AND  CD_TIPO_BILANCIO       = cdTipoBilancio
  AND  CD_UNITA_ORGANIZZATIVA = v_uo_ente
  AND  CD_PIANO_GRUPPI        = ATTPAS;
