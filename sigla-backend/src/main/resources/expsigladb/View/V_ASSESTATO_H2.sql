...
--
     ...
     FROM (SELECT   a.esercizio, a.esercizio_res, a.cd_centro_responsabilita,
                    ...
                    (SELECT NVL (SUM (x.im_vincolo), 0)
                     FROM pdg_vincolo x
                     WHERE x.esercizio <= a.esercizio
                     AND x.esercizio_res = a.esercizio_res
                     AND x.cd_centro_responsabilita = a.cd_centro_responsabilita
                     AND x.cd_linea_attivita = a.cd_linea_attivita
                     AND x.ti_appartenenza = a.ti_appartenenza
                     AND x.ti_gestione = a.ti_gestione
                     AND x.cd_elemento_voce = a.cd_elemento_voce
                     AND x.fl_attivo = 'Y')	importo_vincoli
               FROM (SELECT voce_f_saldi_cdr_linea.esercizio,
                     ...
                     SELECT   a.esercizio, a.esercizio, a.cd_cdr_assegnatario,
                         ...
                         FROM (SELECT pdg_variazione_riga_gest.esercizio,
                                      ...
                                      pdg_variazione_riga_gest.cd_elemento_voce cd_voce,
                                      DECODE
     ...
     LEFT OUTER JOIN progetto_gest progetto ON progetto.esercizio = v_linea_attivita_valida.esercizio AND progetto.pg_progetto = v_linea_attivita_valida.pg_progetto
     LEFT OUTER JOIN progetto_other_field otherField ON otherField.pg_progetto = progetto.pg_progetto
