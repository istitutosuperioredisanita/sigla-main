...
FROM   TIPO_RAPPORTO A,
       ...
       TERZO D,
       COMUNE E
LEFT OUTER JOIN PROVINCIA F ON F.cd_provincia = E.cd_provincia
LEFT OUTER JOIN REGIONE G ON G.cd_regione = F.cd_regione
...
FROM   TIPO_RAPPORTO A,
       ...
       COMUNE E,
       COMUNE E1
LEFT OUTER JOIN PROVINCIA F ON F.cd_provincia = E1.cd_provincia
LEFT OUTER JOIN REGIONE G ON G.cd_regione = F.cd_regione
WHERE  B.cd_tipo_rapporto = A.cd_tipo_rapporto AND
       C.cd_anag = B.cd_anag AND
       D.cd_anag = C.cd_anag AND
