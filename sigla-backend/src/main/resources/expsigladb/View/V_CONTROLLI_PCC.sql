...
--------------------------------------------------------
CREATE OR REPLACE FORCE VIEW "V_CONTROLLI_PCC" ("IDENTIFICATIVO_SDI", "NUMERO_DOCUMENTO", "DATA_DOCUMENTO", "CODICE_FISCALE", "CODICE_DESTINATARIO",
"DATA_RICEZIONE", "DATA_SCADENZA", "STATO_DOCUMENTO", "IMPORTO_DOCUMENTO", "IMPONIBILE", "IMPOSTA", "CD_TERZO", "DS_TERZO", "TIPO_DOCUMENTO",
"ESERCIZIO", "CD_UNITA_ORGANIZZATIVA", "PG_FATTURA_PASSIVA", "DT_REGISTRAZIONE", "STATO_LIQUIDAZIONE", "CAUSALE", "DT_INIZIO_SOSPENSIONE",
"DT_EMISSIONE_MAN", "IM_MANDATO", "IM_TOTALE_NC", "CD_TIPO_CONTRATTO", "FL_IRREGISTRABILE", "CD_UO_CUU", "FL_DA_COMPENSO", "ESITO_PCC",
"FL_SPLIT_PAYMENT", "DA_PAGARE", "ESTERO", "TI_ISTITUZ_COMMERC") AS
  SELECT
    "IDENTIFICATIVO_SDI",
    "NUMERO_DOCUMENTO",
    "DATA_DOCUMENTO",
...
SELECT NULL IDENTIFICATIVO_SDI, fp.NR_FATTURA_FORNITORE NUMERO_DOCUMENTO, fp.DT_REGISTRAZIONE DATA_DOCUMENTO, nvl(fp.CODICE_FISCALE,fp.PARTITA_IVA) CODICE_FISCALE,
       ...
       te.CD_TERZO, te.DENOMINAZIONE_SEDE DS_TERZO,
...
LEFT JOIN TERZO te ON te.CD_TERZO = fp.CD_TERZO
WHERE fp.IDENTIFICATIVO_SDI IS NULL
AND fp.DT_CANCELLAZIONE IS NULL
UNION ALL
SELECT y.IDENTIFICATIVO_SDI, det.NUMERO_DOCUMENTO, det.DATA_DOCUMENTO, nvl(det2.PRESTATORE_CODICEFISCALE,PRESTATORE_CODICE) CODICE_FISCALE,
       ...
       y.IMPONIBILE, y.IMPOSTA,
       nvl(te.CD_TERZO, teele.CD_TERZO), nvl(te.DENOMINAZIONE_SEDE, teele.DENOMINAZIONE_SEDE) DS_TERZO,
...
FROM (SELECT x.ID_PAESE, x.ID_CODICE, x.IDENTIFICATIVO_SDI, x.PROGRESSIVO,
	  ...
	  FROM (SELECT dei.ID_PAESE, dei.ID_CODICE, dei.IDENTIFICATIVO_SDI, dei.PROGRESSIVO, dei.IMPONIBILE_IMPORTO, dei.IMPOSTA,
			...
			SELECT fp.ID_PAESE, fp.ID_CODICE, fp.IDENTIFICATIVO_SDI, fp.PROGRESSIVO, NULL, NULL,
				   ...
				   NULL, NULL, NULL, NULL
			FROM FATTURA_PASSIVA_RIGA fpr
			LEFT OUTER JOIN mandato_riga mr
			    ON mr.CD_CDS = fpr.CD_CDS_OBBLIGAZIONE
				AND mr.ESERCIZIO_OBBLIGAZIONE = fpr.ESERCIZIO_OBBLIGAZIONE
				AND mr.ESERCIZIO_ORI_OBBLIGAZIONE = fpr.ESERCIZIO_ORI_OBBLIGAZIONE
				AND mr.PG_OBBLIGAZIONE = fpr.PG_OBBLIGAZIONE
				AND mr.PG_OBBLIGAZIONE_SCADENZARIO = fpr.PG_OBBLIGAZIONE_SCADENZARIO
				AND mr.ESERCIZIO_DOC_AMM = fpr.ESERCIZIO
				AND mr.CD_CDS_DOC_AMM = fpr.CD_CDS
				AND mr.CD_UO_DOC_AMM = fpr.CD_UNITA_ORGANIZZATIVA
                AND mr.PG_DOC_AMM = fpr.PG_FATTURA_PASSIVA
			LEFT JOIN FATTURA_PASSIVA fp
			    ON fp.ESERCIZIO = fpr.ESERCIZIO
			...
			AND fp.FL_FATTURA_COMPENSO = 'N'
			AND nvl(m.stato, 'N') != 'A'
			AND nvl(mr.CD_TIPO_DOCUMENTO_AMM,'FATTURA_P') = 'FATTURA_P'
			AND fp.DT_CANCELLAZIONE IS NULL
			UNION ALL
			SELECT fp.ID_PAESE, fp.ID_CODICE, fp.IDENTIFICATIVO_SDI, fp.PROGRESSIVO, fpr.IM_IMPONIBILE IMPONIBILE_IMPORTO, fpr.IM_IVA IMPOSTA,
				   m.DT_EMISSIONE,
				   CASE WHEN ((m.PG_MANDATO IS NOT NULL AND m.stato != 'A') OR fpr.STATO_COFI = 'P') THEN
                   	   		CASE WHEN fp.FL_SPLIT_PAYMENT = 'Y' THEN fpr.IM_IMPONIBILE  ELSE fpr.IM_IMPONIBILE + fpr.IM_IVA  END
                   ELSE 0 END IM_MANDATO,
				   NULL, NULL, NULL, NULL
			FROM FATTURA_PASSIVA_RIGA fpr
			...
			UNION ALL
			-- Note collegate solo in ECO e non in FIN
			SELECT fp.ID_PAESE, fp.ID_CODICE, fp.IDENTIFICATIVO_SDI, fp.PROGRESSIVO, NULL, NULL, NULL, NULL,
				   NVL(DECODE(nc.TI_FATTURA, 'C',
                   				            ncr.IM_IMPONIBILE + CASE WHEN fp.FL_SPLIT_PAYMENT = 'Y' THEN 0 ELSE ncr.IM_IVA END,
                   				            -(ncr.IM_IMPONIBILE + CASE WHEN fp.FL_SPLIT_PAYMENT = 'Y' THEN 0 ELSE ncr.IM_IVA END)),0),
			...
			FROM FATTURA_PASSIVA_RIGA fpr
			LEFT JOIN FATTURA_PASSIVA fp ON fp.CD_CDS = fpr.CD_CDS AND fp.CD_UNITA_ORGANIZZATIVA = fpr.CD_UNITA_ORGANIZZATIVA AND fp.ESERCIZIO = fpr.ESERCIZIO AND fp.PG_FATTURA_PASSIVA = fpr.PG_FATTURA_PASSIVA
			LEFT JOIN FATTURA_PASSIVA_RIGA ncr
			    ON ncr.CD_CDS_ASSNCNA_ECO = fpr.CD_CDS
			    ...
			    AND ncr.CD_CDS_ASSNCNA_FIN IS NULL
			    AND ncr.CD_UO_ASSNCNA_FIN IS NULL
			    AND ncr.ESERCIZIO_ASSNCNA_FIN IS NULL
			    AND ncr.PG_FATTURA_ASSNCNA_FIN IS NULL
			    AND ncr.PG_RIGA_ASSNCNA_FIN IS NULL
			LEFT JOIN FATTURA_PASSIVA nc ON ncr.CD_CDS = nc.CD_CDS AND ncr.CD_UNITA_ORGANIZZATIVA = nc.CD_UNITA_ORGANIZZATIVA AND
			ncr.ESERCIZIO = nc.ESERCIZIO AND ncr.PG_FATTURA_PASSIVA = nc.PG_FATTURA_PASSIVA
			WHERE fp.IDENTIFICATIVO_SDI IS NOT NULL
			AND fp.DT_CANCELLAZIONE IS NULL
			...
			FROM FATTURA_PASSIVA_RIGA fpr
			JOIN FATTURA_PASSIVA fp ON fp.CD_CDS = fpr.CD_CDS AND fp.CD_UNITA_ORGANIZZATIVA = fpr.CD_UNITA_ORGANIZZATIVA AND fp.ESERCIZIO = fpr.ESERCIZIO AND fp.PG_FATTURA_PASSIVA = fpr.PG_FATTURA_PASSIVA
			JOIN FATTURA_PASSIVA_RIGA ncr ON ncr.CD_CDS_ASSNCNA_FIN = fpr.CD_CDS AND ncr.CD_UO_ASSNCNA_FIN = fpr.CD_UNITA_ORGANIZZATIVA
			    AND ncr.ESERCIZIO_ASSNCNA_FIN = fpr.ESERCIZIO AND ncr.PG_FATTURA_ASSNCNA_FIN = fpr.PG_FATTURA_PASSIVA
			    AND ncr.PG_RIGA_ASSNCNA_FIN = fpr.PROGRESSIVO_RIGA
			JOIN REVERSALE_RIGA rr ON rr.CD_CDS = ncr.CD_CDS_ACCERTAMENTO AND rr.ESERCIZIO_ACCERTAMENTO = ncr.ESERCIZIO_ACCERTAMENTO
			    AND rr.ESERCIZIO_ORI_ACCERTAMENTO = ncr.ESERCIZIO_ORI_ACCERTAMENTO AND rr.PG_ACCERTAMENTO = ncr.PG_ACCERTAMENTO
			    AND rr.PG_ACCERTAMENTO_SCADENZARIO = ncr.PG_ACCERTAMENTO_SCADENZARIO
			WHERE fp.IDENTIFICATIVO_SDI IS NOT NULL
			AND fp.DT_CANCELLAZIONE IS NULL
...
LEFT JOIN TERZO te ON te.CD_TERZO = fp.CD_TERZO
LEFT JOIN TERZO teele ON teele.CD_TERZO = det2.PRESTATORE_CD_TERZO
)
/
